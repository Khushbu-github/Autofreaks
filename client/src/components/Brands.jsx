...
const Brands = () => {
    return (
        <section className="py-12 sm:py-16 lg:py-20 bg-black/40 overflow-hidden border-y border-white/5 relative">
            {/* Gradient Mask for Fade Effect */}
            <div className="absolute inset-y-0 left-0 w-20 sm:w-40 bg-gradient-to-r from-black to-transparent z-10"></div>
            <div className="absolute inset-y-0 right-0 w-20 sm:w-40 bg-gradient-to-l from-black to-transparent z-10"></div>

            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 sm:mb-12" data-aos="fade-up">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h2 className="text-xs sm:text-sm font-bold text-red-600 tracking-[0.3em] uppercase mb-2">Heritage</h2>
                        <p className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white italic">Our Elite Collection</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-wrap justify-center gap-6 sm:gap-8 md:gap-12">
                    {brands.map((brand, index) => (
                        <div
                            key={index}
                            className="flex flex-col items-center justify-center p-3 sm:p-4 group cursor-pointer w-[calc(50%-1.5rem)] sm:w-[calc(33.33%-2rem)] md:w-[calc(20%-2rem)] lg:w-[calc(14.28%-2rem)]"
                            data-aos="fade-up"
                            data-aos-delay={index * 30}
                        >
                            <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center mb-3 sm:mb-4 transform group-hover:scale-110 transition-transform bg-black rounded-xl p-3 sm:p-4 border border-gray-800 group-hover:border-red-600/50 group-hover:bg-black group-hover:shadow-[0_0_15px_rgba(220,38,38,0.3)] duration-300">
                                <img
                                    src={brand.logo}
                                    alt={`${brand.name} logo`}
                            ...
                            <span className="text-[10px] sm:text-xs font-bold text-gray-500 tracking-[0.2em] uppercase opacity-70 group-hover:opacity-100 group-hover:text-red-500 transition-all duration-300 text-center">
                                {brand.name}
                            </span>
                        </div>
                    ))}
